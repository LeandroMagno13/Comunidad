...
function main() {
  ...
  const md = [
    ...
    '8. **Monte Carlo (200 corridas, seed 101):** %sat media **' + (mcSat.length ? (mcSat.reduce((a: number, b: number) => a + b, 0) / mcSat.length).toFixed(1) : '-') + '%** [p05=' + (mcSat.length ? percentile(mcSat, 0.05).toFixed(1) : '-') + ', p95=' + (mcSat.length ? percentile(mcSat, 0.95).toFixed(1) : '-') + ']; dignidad headcount media **' + (mcDig.length ? (mcDig.reduce((a: number, b: number) => a + b, 0) / mcDig.length).toFixed(1) : '-') + '%** (p95 ' + (mcDig.length ? percentile(mcDig, 0.95).toFixed(1) : '-') + '%, max ' + (mcDig.length ? Math.max(...mcDig) : '-') + '%). El 65,7% de satisfacción NO es un KPI de producto: es un punto de 200 simulaciones bajo una distribución concreta de parámetros y shocks. Lo informativo es la forma de la respuesta (oferta, demanda, población, participación, automatización, patrimonio, urgencia), no el número absoluto.',
    '',
    ...
    '| E15 D (sistema casi vacío) | ' + (by('E15-casi-vacio') ? by('E15-casi-vacio')!.metrics.excluidosBasicoPct : '-') + '% | ' + (by('E15-casi-vacio') ? by('E15-casi-vacio')!.metrics.dignidadHeadcountPct : '-') + '% | ' + (by('E15-casi-vacio') ? by('E15-casi-vacio')!.metrics.dignidadBrecha : '-') + ' |',
    '',
